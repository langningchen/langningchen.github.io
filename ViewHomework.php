<?php
    ...
    if (isset($_POST["HomeworkUploadID"])) {
        if (isset($_POST["Good"])) {
            $Temp = $Connect->prepare("UPDATE Homeworkuploadlist SET Status=5 WHERE HomeworkUploadID=?");
            $Temp->bind_param("i", $_POST["HomeworkUploadID"]);
            $Temp->execute();
        }
        if (isset($_POST["Accept"])) {
            $Temp = $Connect->prepare("UPDATE Homeworkuploadlist SET Status=4 WHERE HomeworkUploadID=?");
            $Temp->bind_param("i", $_POST["HomeworkUploadID"]);
            $Temp->execute();
        }
        if (isset($_POST["Rewrite"])) {
            $Temp = $Connect->prepare("UPDATE Homeworkuploadlist SET Status=2 WHERE HomeworkUploadID=?");
            $Temp->bind_param("i", $_POST["HomeworkUploadID"]);
            $Temp->execute();
        }
    }
    if (isset($_POST["Check"]) && isset($_POST["Data"])) {
        ...
        $Temp = $Connect->prepare("INSERT INTO Homeworkuploadchecklist(HomeworkUploadID, UploadUID, Data) VALUES (?, ?, ?)");
        $Temp->bind_param("iis", $_POST["HomeworkUploadID"], $_SESSION["UID"], $_POST["Data"]);
        $Temp->execute();
    }
    ...
    for ($i = 0; $i < $Result->num_rows; $i++) {
        ...
        if ($RowData[4] != "") {
            ...
            for ($j = 0; $j < count($TempArray); $j++) {
                ...
                if ($FileResult->num_rows == 1) {
                    ...
                    if (gettype(strpos($FileRowData[4], "jpg")) == "boolean" && 
                        gettype(strpos($FileRowData[4], "jpeg")) == "boolean" && 
                        gettype(strpos($FileRowData[4], "png")) == "boolean" && 
                        gettype(strpos($FileRowData[4], "ico")) == "boolean" && 
                        gettype(strpos($FileRowData[4], "bmp")) == "boolean" && 
                        gettype(strpos($FileRowData[4], "gif")) == "boolean" && 
                        gettype(strpos($FileRowData[4], "webp")) == "boolean"
                    ) {
                        array_push($FileList, array("HomeworkUploadFile/" . $FileRowData[0] . "_" . $FileRowData[1] . "_" . $FileRowData[2] . "_", $FileRowData[4]));
                    }
                    else {
                        echo "<img onclick=\"Draw(this, " . $RowData[0] . ")\" class=\"HomeworkImage\" src=\"" . "HomeworkUploadFile/" . $FileRowData[0] . "_" . $FileRowData[1] . "_" . $FileRowData[2] . "_" . $FileRowData[4] . "\" />";
                    }
                }
                else {
        ...
        echo "</form>";
        $CheckTemp = $Connect->prepare("SELECT * FROM Homeworkuploadchecklist WHERE HomeworkUploadID=? ORDER BY CheckTime DESC");
